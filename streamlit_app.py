...
def preprocesar_datos(latitud, longitud, terreno, construccion, habitaciones, banos, modelos):
    logger.debug(f"Preprocesando datos para tipo: {st.session_state.tipo_propiedad}")
    try:
        ...
        logger.debug(f"Datos imputados: {datos_imputados}")
        datos_escalados = modelos['escalador'].transform(datos_imputados)
        logger.debug(f"Datos escalados: {datos_escalados}")
...
def predecir_precio(datos_procesados, modelos):
    ...
    try:
        ...
        # Remove the fixed scaling factor and apply different logic based on property type
        ...
        logger.debug(f"Precio final calculado: {precio_redondeado}")
